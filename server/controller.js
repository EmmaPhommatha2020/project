...
module.exports = {
    ...
    bcrypt.genSalt(10, (err, salt) => {
      bcrypt.hash(password, salt, (err, hash) => {
        req.app
          ...
          .then(user => {
            console.log("checkUsers--->",user[0]);
            if (user[0]) {
              console.log("checkUsers ID--->", user[0].id);
              ...
              req.app
                ...
                .then(orderInfo => {
                  if (orderInfo[0]) {
                    ...
                    console.log("user orderID--->",req.session.user.orderId);
                  } else {
            ...
            bcrypt.hash(password, salt, (err, hash) => {
              req.app
                ...
                .then(user => {
                  ...
                  req.session.user.id = user[0].id;
                  req.session.user.loggedIn = true;
                  console.log("non-active orderID--->", user[0].id);
                  req.app
                    .get("db")
                    .createNewOrder([user[0].id])
                    .then(info => {
                      req.session.user.orderId = info[0].id;
                      console.log("sessionUsers--->",req.session.user);
  ...
  addToCart: (req, res) => {
    ...
    console.log("addToCart--->",req.params, req.session.user.orderId, req.body);
  ...
  payment: (req, res, next) => {
    var charge = stripe.charges.create(
      {
        ...
        description: "Test charge from Laos Shop"
      }
      // , function(err, charge) {
      //     if(err) {
      //         res.sendStatus(500)
    ...
    req.app
      ...
      .then(resp => {
        ...
        console.log("non-active orderId", id);
        req.app
          .get("db")
          .createNewOrder(id)
          .then(info => {
  ...
  updateTotal: (req, res) => {
    var { total } = req.body;
    var { id } = req.session.user;
    console.log("updateTotal--->",req.body.total, req.session.user.id);
    req.app
      .get("db")
